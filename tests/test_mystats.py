...
def test_statistics_functions():
    ...
    assert my.mean(*values) == pytest.approx(np.mean(values))

    assert my.median(*values) == pytest.approx(np.median(values))
    ...
    assert my.data_range(*values) == pytest.approx(np.ptp(values))

    assert my.std_variance(*values) == pytest.approx(np.var(values, ddof=0))

    assert my.sample_variance(*values) == pytest.approx(np.var(values, ddof=1))

    assert my.std_deviation(*values) == pytest.approx(np.std(values, ddof=0))

    assert my.sample_deviation(*values) == pytest.approx(np.std(values, ddof=1))
